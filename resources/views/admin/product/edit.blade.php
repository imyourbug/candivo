...
@section('title', 'Edit Product – Di-tool Admin')
...
@section('content')
    <header class="flex items-center justify-between mb-8">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Product</h2>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden max-w-4xl">
        <form id="admin-product-form" action="{{ route('admin.products.update', $product) }}" method="post" enctype="multipart/form-data" class="p-6 md:p-8" novalidate>
            @csrf
            @method('PUT')
