@php
    ...
    $imagesFieldValue = old('images');
    if ($imagesFieldValue === null && $isEdit) {
        $imgs = $package->images;
        $imagesFieldValue = is_array($imgs) ? implode(',', $imgs) : (string) $imgs;
    }
    $imagesFieldValue = $imagesFieldValue ?? '';

    $packageImageUrls = $isEdit ? array_values(array_filter(array_map('trim', $package->images ?? []))) : [];
@endphp
...
<div class="space-y-6">
    ...
    <div class="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-6">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Upload an avatar and gallery images, or paste external URLs.</p>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="space-y-3">
                <span class="block text-sm font-semibold text-slate-700 dark:text-slate-300">Avatar</span>
                @if($isEdit && filled(old('avatar', $package?->avatar)))
                    <p class="text-xs font-semibold text-slate-600 dark:text-slate-400">Saved avatar</p>
                    <div class="flex items-start gap-3">
                        <img src="{{ old('avatar', $package->avatar) }}" alt="" class="h-20 w-20 rounded-lg object-cover border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800" />
                    </div>
                @endif
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 mb-2">Choose multiple files; they are appended to the list after save.</p>
            ...
            <div class="mt-3">
                ...
                <textarea name="images" id="images" rows="3"
                    class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary @error('images') border-rose-500 @enderror">{{ $imagesFieldValue }}</textarea>
                @error('images')
                    <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                @enderror
            </div>
            @if(count($packageImageUrls) > 0)
                <p class="text-xs font-semibold text-slate-600 dark:text-slate-400 mt-3 mb-2">Current gallery</p>
                <div class="flex flex-wrap gap-2">
                    @foreach($packageImageUrls as $url)
