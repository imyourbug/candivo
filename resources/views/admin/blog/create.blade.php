...
@section('title', 'New Post – Di-tool Admin')
...
@section('content')
    <header class="flex items-center justify-between mb-8">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-900 dark:text-white">New Post</h2>
            <p class="text-slate-500 text-sm">Create a new blog post.</p>
        </div>
    </header>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden max-w-4xl">
        <form action="{{ route('admin.posts.store') }}" method="post" class="p-6 md:p-8" id="blog-form">
            @csrf
